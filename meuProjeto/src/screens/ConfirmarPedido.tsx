...
export default function ConfirmarPedido({ route, navigation }: any) {
  ...
  const REAIS_POR_PONTO = 5; // 1 ponto a cada R$5 gastos
  ...
  async function finalizarPedido() {
    ...
    // Impedir vendedor de comprar seus próprios produtos
    ...
    try {
      ...
      if (pedidosPorVendedor.size === 0) {
        Alert.alert("Erro", "Nenhum vendedor identificado");
        return;
      }
      ...
      for (const [vendedorId, itens] of pedidosPorVendedor) {
        ...
        const pedido = {
          ...
          avaliado: false, // Campo para evitar dupla avaliação
          criadoEm: new Date(),
        };
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.section}>
        ...
        {metodosPagamento.map((metodo) => (
          <TouchableOpacity key={metodo.id} style={[styles.pagamentoOption, metodoPagamento === metodo.id && styles.pagamentoOptionSelected]} onPress={() => setMetodoPagamento(metodo.id)}>
      ...
      <TouchableOpacity style={[styles.botaoFinalizar, loading && styles.botaoDisabled]} onPress={finalizarPedido} disabled={loading}>
